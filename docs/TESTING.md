...
| `node run-snapshot-regressions.cjs` | [Snapshot-ajuri](../run-snapshot-regressions.cjs), 41 tarkistusta: metadata ja HTML-escaping, V1/V2/V3, versio/digest/esitystarkiste, syväkopiointi, hakukielto katselussa, save/open/rename/delete-controllerit, vahvistus/peruutus, korruptio ja aktiivisen työn eristys; 50 tilauksen koon mittaus | VM, DOM-/store-testikaksoiset; IndexedDB:n transaktiot tarkistetaan erikseen selaimessa |
...
Lataa [snapshot-browser-regressions.js](../snapshot-browser-regressions.js) vain erillisellä testi-originilla laskettuun työhön. `await runSnapshotBrowserRegressions()` testaa oikeaa IndexedDB-storea (15 tarkistusta): lisäys, duplikaatti-ID:n hylkäys, metadata-rename, valittu poisto, syväkopio, korruptoitunut entry ja add/put/delete-transaktioiden pakotetut abortit. Se vertaa aktiivista localStoragea tavutasolla ja canonical-live-tilaa ilman vaihtuvaa savedAt-aikaa. Ajuri poistaa finallyssä vain omat UUID-tallenteensa; ennestään olevat kopiot säilyvät. Ajuri ei kuulu tuotantosivun skripteihin.
