...
| `node run-snapshot-regressions.cjs` | [Snapshot-ajuri](../run-snapshot-regressions.cjs), 67 tarkistusta: metadata ja HTML-escaping, V1/V2/V3, versio/digest/esitystarkiste, syväkopiointi, hakukielto katselussa, save/open/rename/delete-controllerit sekä B-014:n canonical restore, turvakopio, fyysiset estot, vahvistus/peruutus ja persistointi-/renderöintivirheet; 50 tilauksen koon mittaus | VM, DOM-/store-testikaksoiset; IndexedDB:n transaktiot tarkistetaan erikseen selaimessa |
...
### Snapshotien selainvarmistus

Lataa [snapshot-browser-regressions.js](../snapshot-browser-regressions.js) vain erillisellä testi-originilla laskettuun työhön. `await runSnapshotBrowserRegressions()` testaa oikeaa IndexedDB-storea (20 tarkistusta): lisäys, tarkka ID-luku, duplikaatti-ID:n hylkäys, metadata-rename, valittu poisto, syväkopio, korruptoitunut entry, add/put/delete-transaktioiden pakotetut abortit sekä B-014:n turvakopion, immutable-kohteen, canonical-palautuksen ja active-store-virheen. Se vertaa aktiivista localStoragea tavutasolla ja canonical-live-tilaa ilman vaihtuvaa savedAt-aikaa. Ajuri poistaa finallyssä vain omat UUID-tallenteensa; ennestään olevat kopiot säilyvät. Ajuri ei kuulu tuotantosivun skripteihin.

12.9.2026 Codex in-app -selaimessa erillisessä `127.0.0.1:8771`-alkuperässä ajettiin D1:n oikea laskenta (6000 mm Pysty, 2200 mm × 2, kerf 3; käyttämätön 3900 mm jäännös): `Snapshot test 1` tallennettiin 0/2-tilassa, aktiivinen työ kuitattiin 1/2:een ja katselu näytti edelleen 0/2. Katselussa oli vain paluupainike, ei tuotanto-ohjaimia. Paluu, reload, rename ja sen reload säilyivät. Erillisen poistotestin oma vahvistusdialogi peruutettiin ensin, sitten hyväksyttiin; reload näytti kopion poistetuksi ja aktiivisen työn edelleen 1/2:ssa. Tumma dialogi tarkistettiin kuvasta.

V3-testissä 7 × 6000 mm musta Pysty, kerf 3 ja 7 → 3 -poikkeama aktivoitiin normaalilla UI:lla. Snapshot tallennettiin jatkon 0/3-tilassa, aktiivinen jatko kuitattiin 1/3:een. Snapshot näytti alkuperäisen kirjatun poikkeaman ja vanhan jatkon 0/3-järjestyksen. Avaus onnistui myös silloin, kun batch-selector, inventory-optimizer ja continuation-haku oli testissä vaihdettu poikkeuksen heittäviksi. Paluu ja reload säilyttivät aktiivisen 1/3-tilan ja snapshotin erillään. Oikean IndexedDB-ajurin kaikki 15 tarkistusta läpäisivät tässä V3-tilassa. Fyysistä puhelinta tai todellista levyn täyttämistä ei testattu; storage-failure katetaan controller-virheillä ja oikeilla transaktio-aborteilla.

### B-014:n snapshot-palautuksen selainvarmistus

Käytä erillistä originia. Tallenna ensin laskettu suunnitelma ilman tapahtumia, muuta aktiivinen työ luonnokseksi ja avaa snapshot. Tarkista käytettävissä oleva **PALAUTA AKTIIVISEKSI TYÖKSI**, kohteen ja aktiivisen työn vahvistusyhteenvedot, peruutuksen muuttumattomuus sekä **LUO TURVAKOPIO JA PALAUTA**. Onnistumisen jälkeen aktiivisen planin, schedulerin ja 0-tapahtuman tilan pitää vastata snapshotin canonical `workState`:a; snapshot-listassa pitää näkyä `Ennen palautusta – <timestamp>` ja reloadin pitää palauttaa sama aktiivinen tila.

Estot tarkistetaan erikseen. Kirjaa aktiiviseen työhön yksi työvaihe ja avaa ennen tapahtumaa tallennettu snapshot: palautuspainike on disabled ja syy nimeää aktiivisen fyysisen toteuman. Luo lisäksi B-013-fixturellä 7 → 3 -poikkeaman V3-jatko, tallenna siitä snapshot, palauta aktiivinen testi-fixture ennen tapahtumaa olevaan tilaan ja avaa V3-snapshot: katselu toimii, mutta palautuspainike on disabled ja syy nimeää snapshotin fyysisen toteuman.

12.9.2026 tämä polku ajettiin Codex in-app -selaimessa originissa `127.0.0.1:8773`. D1:n 6000 mm Pysty / 2200 mm × 2 / kerf 3 palautui luonnoksen päälle 0/2-scheduler-tilassa, automaattinen turvasnapshot syntyi ja reload säilytti planin. Sama turvasnapshot palautettiin myöhemmin tapahtumattoman planin päälle: vanha tuotantonäkymä poistui ja luonnos säilyi reloadissa. Aktiivisen 1/2-tapahtuman esto sekä 7 × 6000 mm mustan Pysty-fixturen in-progress V3-kohteen esto näkyivät erillisillä syillä. Samassa originissa oikean IndexedDB-ajurin 20/20 tarkistusta läpäisivät. Käyttäjän oikeaa työtä ei käytetty.
