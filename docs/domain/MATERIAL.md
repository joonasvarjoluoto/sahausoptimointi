...
`calculatePostOrderMaterialInventory()` vähentää käytetyt äärelliset uudet salot ja vanhat jäännökset, säilyttää käyttämättömät lähteet sekä lisää vain dispositionin säästettävät turvalliset loppujäännökset ryhmiteltyinä. Saman salon jatkoleikkauksista ei tehdä useaa varastovähennystä. Tuotannon finalisointitransaktio on kuvattu tuotantomallissa. Tutkimuksen profiilikohtaiset fyysiset säilytysrajat eivät ole tämän funktion sääntö.
