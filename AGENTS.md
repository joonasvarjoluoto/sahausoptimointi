...
Tuotannon suoritus-UI (10.9.2026): `src/production-integration.js` johtaa peräkkäisten identtisten operaatioiden toistoryhmät schedulerin alkuperäisestä listasta. Identiteetti vertaa lajia, mittaa, nippusääntöä, järjestettyjä fyysisiä sourceId/profiili/väri-tietoja sekä kappaleiden lähde-, tilaus- ja aukkokohdistusta. Vaihtuvat operation-/piece-ID:t, salon lyheneminen ja saman salon jatkoleikkauksen origin eivät katkaise ryhmää. Yksi kuittaus tarkoittaa aina yhtä alkuperäistä scheduler-operaatiota. Ryhmän `x / n` johdetaan `events.length`-arvosta; ryhmää tai laskuria ei tallenneta, eikä planDigest muutu. Reload ja undo käyttävät samaa tapahtumalokia.

Näkymä näyttää enintään kolme viimeksi kuitattua operaatiota, nykyisen kortin/toistoryhmän ja kolme operaatiota nykyisen ryhmän jälkeen. Valmisteluyhteenveto näyttää koko aktiivisen profiiliblokin lähteet profiilin, värin ja lähdetyypin mukaan. Worker-numerot ja jäännösten nimellispituudet säilyvät näkyvissä. Ohjelma näyttää materiaalitarpeen; operaattori päättää itse kantomäärän ja varastoreissut.
