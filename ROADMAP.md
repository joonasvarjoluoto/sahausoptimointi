...
Suoritusnäkymän toistopalaute toteutettu 10.9.2026. Identtiset peräkkäiset scheduler-operaatiot näkyvät yhden pääkortin toistoryhmänä ja `x / n tehty` -laskurina. Yksi kuittaus lisää edelleen yhden alkuperäisen operaation eventin; reload ja undo johtavat näkymän tästä etuliitteestä. Lähihistoria näyttää kolme viimeksi tehtyä ja esikatselu kolme operaatiota nykyisen ryhmän jälkeen. Aktiivisen profiiliblokin koko materiaalitarve ryhmitellään profiilin, värin ja lähdetyypin mukaan worker-numeroineen ja jäännöspituuksineen. Kantomäärän päättää operaattori. Scheduler, planDigest, materiaalimoottori ja skeema säilyvät ennallaan.

Varmistus: Node 38/38 ryhmää, ohjaus-/persistenssi 45 tarkistusta, neljä täydellistä material-fixturea sekä 1 354 kuviota ja 15 kokonaista benchmark-suunnitelmaa. Edge-selaimessa musta Pysty 1300 mm × 16, musta Vaaka 1000 mm × 20 ja harmaa Pysty 1300 mm × 10 sekä harmaat Pysty-jäännökset 1740/1510 mm tuottivat neljän identtisen sahausliikkeen ryhmän. Kuittaus, reload, undo, ryhmäraja, 3+3-jono ja valmisteluyhteenveto testattiin Edgen 390 × 844 tummassa puhelinemulaatiossa. Asettelu tarkistettiin myös 1280 × 900 vaaleassa näkymässä; fyysistä puhelinta ei käytetty. Selain käytti erillistä tilapäistä HTTP-alkuperää ja profiilia.

Materiaalikapasiteetin turvavarat toteutettu 10.9.2026. Uuden ja jäännöslähteen nimellispituus säilyy, mutta optimizer vähentää turvallisesta kapasiteetista alustavasti 20 mm lähdettä kohti ja 1 mm kappaletta kohti. Kerf pysyy erillisenä. Suunnitelma erottaa turvallisen `remaining`-jäännöksen nimellisestä `nominalRemaining`-loppupituudesta ja kapasiteettivarasta; finalisointi palauttaa varastoon vain turvallisen pituuden. Skeema säilyy versiossa 6, mutta materiaalimoottori nousee versioon `material-v0.4`. Turvallinen kapasiteetti saa loppua nollaan, jolloin oletusvarojen takia viimeinen kappale on sahausliike eikä `release`-poiminta.
...
| Mittatoleranssit ja erilliset kapasiteettivarat | Lähdevara 20 mm ja kappalevara 1 mm toteutettu erillään kerfistä; arvot odottavat tuotantodatalla kalibrointia |
...
5. **Irrota inventory/material-logiikka pienissä osissa — ensimmäinen irrotus toteutettu 10.9.2026.** `src/material.js` sisältää `PROFILE_TYPES`-määrittelyn, kapasiteettiasetusten normalisoinnin ja materiaalitangon kapasiteettitaseen sekä `isSupportedProfileType()`, `validateMaterialAvailability()`, `createMaterialInventory()`, `getMaterialSourcesForProfile()`, `consumeMaterialSource()` ja `calculateMaterialUsage()` -funktiot. Jälkivaraston muodostus ja jäännösten arvotus jäävät vielä `app.js`:ään.
...
Nykyinen latausjärjestys on cutting-physics → material → production-planning → production-integration → app tavallisina skripteinä. Samat tiedostot ladataan Node-ajureissa ja tutkimustyökaluissa; historiallisia tutkimustuloksia ei muuteta. `MATERIAL` on jäädytetty rajapinta ja myös CommonJS-ladattava. Sovelluksen globaalit funktiot säilyvät `var`-aliasina ja profiilimäärittely `const`-aliasina. Skeema on 6 ja kapasiteettivaramallin materiaalimoottoriversio `material-v0.4`.

Materiaalirajan vertailu: ennen siirtoa tallennettiin commitin `54445d7` neljän perusfixturen täydet optimoinnit, suunnitelmat, varastot, piste-erittelyt ja operaatiot. Rakenteellisen irrotuksen jälkeen tulos säilyi identtisenä. Kapasiteettivaramallin tarkoituksellinen käyttäytymismuutos päivitti `run-material-regressions.cjs`-checkpointin: D1:n turvallinen jäännös on 1572 mm, kun nimellinen loppupituus on edelleen 1594 mm. Nykyinen core-paketti sisältää erillisen kahdeksan kohdan kapasiteettiregression.

Ennen kapasiteettivaramallia tehty HTTP-selaintesti 10.9.2026 käytti mustaa Pystyä 2200 mm × 2, vanhaa jäännöstä 3900 mm, rajatonta uutta 6000 mm materiaalia ja 3 mm kerfiä. Silloinen 1594 mm jäännös on uuden mallin vertailuarvo: nimellinen loppupituus pysyy 1594 mm:nä, mutta turvallinen ja varastoon palautettava pituus on 1572 mm.
